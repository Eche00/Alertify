...
// ✅ Unified Symbol Map — aligns mismatched IDs to proper tickers
...
// 🪙 CoinGecko coin list
const CHAINLINK_COINS = Object.keys(SYMBOL_MAP);

// Derived RedStone and Pyth symbols
const REDSTONE_SYMBOLS = Object.values(SYMBOL_MAP);
const PYTH_SYMBOLS = Object.values(SYMBOL_MAP);
...
async function fetchChainlinkPrices() {
  try {
    ...
    return CHAINLINK_COINS.reduce((acc, c) => {
      const sym = SYMBOL_MAP[c];
      acc[sym] = { oracle: "Chainlink", price: null, status: "Failed", updated: new Date().toISOString() };
      return acc;
...
async function fetchPythPrices() {
  try {
    ...
    // Filter only feeds with matching symbols and /USD pairs
