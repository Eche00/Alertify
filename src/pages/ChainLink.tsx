...
export default function ChainlinkPage({ onFeedsUpdate }: ChainLinkProps) {
  ...
  const lastUpdated =
    withChange.length > 0 ? withChange[withChange.length - 1].updated : null;
    ...
    <div className="w-full mt-10">
      <h1 className="text-2xl font-bold mb-6 flex items-center justify-between">
        ...
        <div className="flex flex-col items-end justify-end gap-2">
          ...
          <p className="text-end text-[16px] font-light ">
            Last updated: <br className="sm:hidden block" /> {lastUpdated}
          </p>
        </div>
      </h1>
  ...
  className={`px-3 py-1 border rounded-md text-sm cursor-pointer transition-all duration-300 flex items-center gap-1 
    ${
      ...
      JSON.parse(localStorage.getItem("alerts") || "[]").some(
        (a: any) => a.asset === item.asset && a.oracle === item.oracle
      )
